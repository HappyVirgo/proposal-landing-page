...
let columnsNotes = [ 
    {
        label: "",
        dataKey_wo: "workOrderNotes",
        dataKey_wo_date: "createdAt",
        dataKey_wo_user: "serviceProvider",
        dataKey_wo_company: "companyName",
        dataKey_prps: "proposalNotes",
        dataKey_prps_date: "dateCreated",
        dataKey_prps_user: "serviceProvider",
        dataKey_prps_company: "companyName",
        dataKey_invs: "invoiceNotes",
        dataKey_invs_date: "createdAt",
        dataKey_invs_user: "serviceProvider",
        dataKey_invs_user_profile: "userProfile",
...
const CommonTable = ({tmpdata, infotab}) => {
    ...
    if(tmpdata!==undefined){
            ...
            if (tmpdata.data.work_order_histories!==undefined) {
                ...
                let wo_notes
                wo_notes = !tmpdata.data.response['workOrderNotes']?wo_notes=[]:tmpdata.data.response['workOrderNotes']
                wo_notes.map((note) => {
                    note.workOrderNotes = "workOrderNotes";
                    
                    return wo_notes
                })             
                let prp_notes
                prp_notes = !tmpdata.data.response['proposalNotes']?prp_notes=[]:tmpdata.data.response['proposalNotes']
                prp_notes.map((note) => {
                    note.proposalNotes = "proposalNotes";
                    
                    return prp_notes
                })                
                let inv_notes = tmpdata.data.response['invoiceNotes']
                inv_notes = !tmpdata.data.response['invoiceNotes']?inv_notes=[]:tmpdata.data.response['invoiceNotes']
                inv_notes.map((note) => {
                    note.invoiceNotes = "invoiceNotes";
